...
if os.path.exists(file):
    try:
        with open(file, 'rt') as input_file:
            ...
            environment['CONNECT_GROUP_ID'] = 's3csv'
            environment['CONNECT_CONFIG_STORAGE_TOPIC'] = 's3csv-storage'
            environment['CONNECT_STATUS_STORAGE_TOPIC'] = 's3csv-status'
            environment['CONNECT_OFFSET_STORAGE_TOPIC'] = 's3csv-offset'
            #print(environment)
        input_file.close()
    except:
